 ...
 *
 * Once the core has run, we will attempt to exit, since it is done.
 */
void retro_run(void)
{
    // Clear the display.
    ...
    // Shutdown the environment now that it has loaded and quit.
...
bool retro_load_game(const struct retro_game_info *info)
{
    ...
    if (info != NULL && info->path != NULL && info->path[0] != '\0') {
        // sprintf(command, ". \"%s\"", info->path);
        sprintf(command, "bash -c \'. \"%s\"\'", info->path);
    }

    if (system(command) == 0) {
        printf("libretro-bash-launcher: Finished executing bash scripts\n");
        return true;
    }

    printf("libretro-bash-launcher: Failed executing bash scripts.\n");
    return false;
}
